...
- 長時間の作業を未コミットのまま積み上げない。**独立して動作確認できる修正が1つ完成するたびに、その場でコミットする**
- 次の変更へ着手する前に、直前の変更をテストし、コミット可能な状態へ閉じる。UI、データ処理、テスト基盤など異なる系統を1つの未コミット差分へ混在させない
- 実機またはSimulatorでユーザーが正常動作を確認した状態は、追加修正へ進む前に必ずチェックポイントコミットとして保存する
- 未完成コード、ビルド不能、既知の回帰を含む状態はコミットしない。作業退避が必要な場合も、正常な直前コミットを保持した上で行う
- 既存のユーザー変更が混在して分割できない場合は、勝手に一括コミットせず、`git diff`で境界を特定して関連hunkだけをstageする
- コミット前に`git diff --cached`で対象範囲を確認し、コミット後に`git status --short`で残差分を報告する
- 目安として、以下のいずれかに達したらチェックポイントを切る
  - 1件の不具合修正と対応する回帰テストが通った
  - 1つの画面またはコンポーネントの変更が実表示で確認できた
  - 30分以上継続する作業で、復元可能な正常状態ができた
