...
struct PhotoImportServiceTests {
    @Test
    func imageProcessorDownsamplesBeforeDecodeAndReturnsDataInput() async throws {
        ...
        let output = try await CardImageProcessingService().prepareInput(
            from: input,
            source: .photoLibrary,
        ...
        #expect(width > height)
        #expect(output.source == .photoLibrary)
    }
}
